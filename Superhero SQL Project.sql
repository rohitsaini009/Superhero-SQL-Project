...
-- What is the distribution of superheroes across different genders? 
SELECT g.gender, COUNT(s.id) AS superhero_count
FROM Superhero AS s
LEFT JOIN Gender AS g ON s.gender_id = g.id
GROUP BY g.gender;


-- How does the distribution of superheroes vary across different races?
select * from race;

select r.race,count(s.id) as superhero_count FROM superhero AS s
left join race as r on s.race_id = r.id
group by r.race
order by superhero_count;

-- Which publisher has the highest number of superheroes?
select * from publisher;

select p.publisher_name,max(s.id) as highest_num from superhero as s
left join publisher as p on s.publisher_id = p.id
group by p.publisher_name
order by highest_num desc
limit 1;

-- What are the most common attributes among superheroes?
select * from superhero;

select a.attribute_name,count(s.id) as most_common_attribute from superhero as s
left join attribute as a on s.attribute_id = a.id
group by a.attribute_name
order by most_common_attribute desc;
...
SELECT a.attribute_name, COUNT(ha.attribute_id) AS attribute_count
FROM Attribute AS a
LEFT JOIN Hero_Attribute AS ha ON a.id = ha.attribute_id
GROUP BY a.attribute_name
ORDER BY attribute_count DESC;

-- Among superheroes with attributes, what is the average rating for each attribute?

SELECT a.attribute_name, round(avg(ha.attribute_id)) AS Average_Rating
FROM Attribute AS a
LEFT JOIN Hero_Attribute AS ha ON a.id = ha.attribute_id
GROUP BY a.attribute_name
ORDER BY Average_Rating DESC;

-- What are the most common superpowers among superheroes?
select * from superpower;
SELECT sp.power_name, COUNT(hp.hero_id) AS superpower_count
FROM superpower AS sp
LEFT JOIN Hero_Power AS hp ON sp.id = hp.hero_id
GROUP BY sp.power_name
ORDER BY superpower_count DESC;

-- How many superheroes possess a specific superpower?

SELECT COUNT(hp.hero_id) AS superhero_count
FROM Hero_Power AS hp
JOIN Superpower AS sp ON hp.hero_id = sp.id
WHERE sp.power_name = 'Energy Armor';


-- Who are the tallest and heaviest superheroes?

select * from superhero;

select superhero_name,max((height_cm)) as tallast from superhero
group by superhero_name
order by tallast desc
limit 1;

select superhero_name,max((weight_kg)) as heaviest from superhero
group by superhero_name
order by heaviest desc
limit 1;


-- What is the distribution of alignment among superheroes?

SELECT 
    a.alignment,
    COUNT(s.id) AS alignment_count
FROM 
    Superhero AS s
LEFT JOIN 
...
-- Do certain publishers have a higher proportion of good, neutral, or bad-aligned superheroes?    
SELECT 
    p.publisher_name,
    a.alignment,
    COUNT(s.id) AS alignment_count
...
LEFT JOIN 
    Alignment AS a ON s.alignment_id = a.id
GROUP BY 
    p.publisher_name, a.alignment;
    
-- How many superheroes have multiple identities or roles (e.g., Green Lantern)?    

SELECT COUNT(DISTINCT superhero_name) AS multi_role_superheroes_count
FROM Superhero;


-- What are the attributes and superpowers associated with each role of multi-role superheroes?
...
FROM 
    Superhero AS s
LEFT JOIN 
    Hero_Attribute AS ha ON s.id = ha.hero_id
LEFT JOIN 
    Attribute AS a ON ha.attribute_id = a.id
LEFT JOIN 
    Hero_Power AS hp ON s.id = hp.hero_id
LEFT JOIN 
    Superpower AS sp ON hp.hero_id = sp.id
WHERE 
    s.superhero_name IN (
        SELECT 
            superhero_name
...
ORDER BY 
    s.superhero_name, a.attribute_name, ha.attribute_value;
    
    
-- How many superheroes do not have attribute values or superpower values?
    

SELECT 
    COUNT(DISTINCT s.id) AS superheroes_without_attributes,
    COUNT(DISTINCT s.id) AS superheroes_without_superpowers
FROM 
    Superhero AS s
LEFT JOIN 
    Hero_Attribute AS ha ON s.id = ha.hero_id
LEFT JOIN 
    Hero_Power AS hp ON s.id = hp.hero_id
WHERE 
    ha.attribute_id IS NULL
    OR hp.power_id IS NULL;
